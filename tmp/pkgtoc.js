...
const {
  ...
  yellow,
  red
} = require("chalk"); // Defaults / Constants
...
const COLUMN_MAX = 3;
const SHIELDS_COLOR = "cb3837"; // Table bookends
...
const getShield = ({
  ...
  return `![${name} npm badge)](${shieldsAuto ? `https://img.shields.io/badge/npm-${fixVersion}-${shieldsColor}.svg` : `https://img.shields.io/npm/v/${name}.svg?colorB=${shieldsColor}`})`;
};
    ...
    case 2:
      return {
        ...
        row: ({
          ...
          shield,
          title
        }) => shields ? [shield, `${title}<br />${description}`] : [title, description]
      };
...
const formatPackagesToTable = (packages, flags) => {
  ...
  return packages.reduce(async (memo, pkgList) => {
    ...
    return (await memo).concat(`**${getPackageType(pkgList[0])}**\n`).concat(reduceArrToRow(head)).concat(reduceArrToDiv(head)).concat(table.sort((a, b) => a.name > b.name ? 1 : -1).map(({
      name,
      description,
      path,
      version
    }) => reduceArrToRow(format.row({
      shield: getShield({ ...flags,
        name,
        version
      }),
      title: getTitle({
...
const getPackagesList = async ({
  ...
  return Promise.all(globsPackages.map(async globPattern => {
    logger.log("Reading", globPattern);
    return glob(globPattern);
  }));
};

const countPackages = packages => packages.reduce((acc, pkgs) => acc + pkgs.length, 0);
...
const pkgtoc = async (_flags = {}) => {
  try {
    // Get flag defaults.
    const flags = getFlags(_flags); // Get readme and packages
    ...
    const indexEnd = contents.indexOf(COMMENT_END); // Push table to beginning if no table comments

    const tableStart = Math.max(0, indexStart);
    const tableEnd = indexEnd > 0 ? indexEnd + COMMENT_END.length : tableStart; // Assemble rows from package.jsons
